...
class MultimodalDataset(torch.utils.data.Dataset):
    def __init__(self, csv_file, audio_dir, video_dir, text_dir, text2_dir, question):
    ...
    def __getitem__(self, idx):
        ...
        batch = {
            ...
            'texts2': torch.tensor(text_features2, dtype=torch.float32),  # [5, text_dim]
            'id': sample_id
        }
        return batch
...
def main():
    ...
    parser.add_argument('--text2_dir', type=str, required=True)
    ...
    test_dataset = MultimodalDataset(args.test_csv, args.audio_dir, args.video_dir, args.text_dir, args.text2_dir, args.question)
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, collate_fn=lambda x: {
        ...
        'texts2': torch.stack([item['texts2'] for item in x]),
    ...
    class Args:
        ...
        grad_clip = 1.0
    model_args = Args()
    model = AVIBASELINE(model_args).to(device)
