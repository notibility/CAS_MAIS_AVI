...
from models.avi_task1 import AVIBASELINE
...
class MultimodalDataset(Dataset):
    def __init__(self, csv_file, audio_dir, video_dir, text_dir, text2_dir, question, label_col):
        self.data = pd.read_csv(csv_file)
        self.audio_dir = audio_dir
        self.video_dir = video_dir
        self.text_dir = text_dir
        self.text2_dir = text2_dir
    ...
    def __getitem__(self, idx):
        ...
        text2_file = [f for f in os.listdir(self.text2_dir) if f.startswith(f"{sample_id}_{self.question}")]

        if len(audio_file) == 0 or len(video_file) == 0 or len(text_file) == 0 or len(text2_file) == 0:
        ...
        text_features2 = np.load(os.path.join(self.text2_dir, text2_file[0]))
        ...
        text_features2 = np.tile(np.expand_dims(text_features2, 0), (5, 1))
        ...
        batch = {
            ...
            'texts2': torch.tensor(text_features2, dtype=torch.float32)   # [5, 768]
        }
...
def main():
    ...
    parser.add_argument('--text2_dir', required=True, help='第二个文本特征的目录路径')
    ...
    # 创建数据集
    train_set = MultimodalDataset(args.train_csv, args.audio_dir, args.video_dir, args.text_dir, args.text2_dir, args.question, args.label_col)
    val_set = MultimodalDataset(args.val_csv, args.audio_dir, args.video_dir, args.text_dir, args.text2_dir, args.question, args.label_col)
    ...
    # 创建模型
    model = AVIBASELINE(args).to(device)
